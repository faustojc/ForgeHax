...
public class PositionRotationManager extends ServiceMod {
  ...
  private static Angle getPlayerAngles(Player player) {
    return Angle.degrees(player.getXRot(), player.getYRot());
  }

  private static void setPlayerAngles(LocalPlayer player, Angle angles) {
    Angle original = getPlayerAngles(player);
    Angle diff = angles.normalize().sub(original.normalize()).normalize();
    player.setXRot(Utils.clamp(original.getPitch() + diff.getPitch(), -90.f, 90.f));
    player.setYRot(original.getYaw() + diff.getYaw());
  }

  private static void setPlayerPosition(LocalPlayer player, Vec3 position) {
  ...
  public void onMovementUpdatePre(LocalPlayerUpdateMovementEvent.Pre event) {
    ...
    // every listener that ran this tick gets its invokeLater tasks run, not just the one that ends
    // up owning the rotation. a listener that only queues work (KillAura with rotate off, for
    // example) never touches the view angles, so it never wins the loop and used to be overwritten
    // by the next listener before its tasks could be collected
    TaskChain.Builder<Consumer<ReadableRotationState>> tasks = TaskChain.builder();

    RotationState.Local ls;
    ...
    for (MovementUpdateListener listener : getManager().functions()) {
      ...
      tasks.collect(ls.getFutureTasks());
    ...
    if (smooth.get() > 0.D) {
      ...
      if (getRotationCount(start, dest, smooth.getAsFloat()) <= 1.f) {
        futureTasks = tasks.build();
      } else {
        // still turning, hold the tasks back so nothing fires on a half finished rotation
  ...
  public void onPacketReceived(PacketEvent.Incoming.Pre event) {
    ...
    if (event.getPacket() instanceof ClientboundPlayerPositionPacket) {
      ...
      final ClientboundPlayerPositionPacket packet = event.getPacket();
  ...
  public interface MovementUpdateListener {
     ...
     *
     * @param state
     *     read/write data for manipulating the players rotation.
     */
  ...
  public interface ReadableRotationState {
    ...
    default Angle getRenderClientViewAngles() {
      return isActive() ? getClientAngles()
          : Angle.degrees(getLocalPlayer().getXRot(), getLocalPlayer().getYRot());
    }
